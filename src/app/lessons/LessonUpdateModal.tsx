...
type Option   = { id?: number | null; startDate: string; startTime: string; endDate: string; endTime: string; region: string; place: string; placeUrl: string };
type Discount = { id?: number | null; type: "E" | "S" | ""; condition: string; amount: string };
type Contact  = { id?: number | null; type: string; account: string; name: string };
type Notice   = { id?: number | null; type: string; text: string };
type FieldError = { field: string; message: string };

type Props = { lessonNo: number; onClose: () => void; onSuccess: () => void };
...
const CONTACT_TYPES = [
  { value: "Y", label: "유튜브" }, { value: "K", label: "카카오" },
  { value: "W", label: "웹사이트" }, { value: "I", label: "인스타그램" },
  { value: "L", label: "링크" }, { value: "M", label: "기타" },
];
const NOTICE_TYPES = [
  { value: "L", label: "링크" }, { value: "T", label: "텍스트" },
  { value: "R", label: "환불" }, { value: "N", label: "공지" }, { value: "U", label: "URL" },
];

export default function LessonUpdateModal({ lessonNo, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setTitle(d.title ?? "");
        setGenre(d.genre ?? "");
        setInstructorLo(d.instructorLo ?? "");
        setInstructorLa(d.instructorLa ?? "");
        setAmount(d.amount != null ? String(d.amount) : "");
        setIsActive(d.isActive ?? true);
        setOptions(
          d.options?.length
            ? d.options.map((o: { id: number | null; startDate: string; startTime: string; endDate: string; endTime: string; region: string; place: string | null; placeUrl: string | null }) => ({
                id: o.id ?? null,
                startDate: o.startDate ?? "", startTime: o.startTime ?? "",
                endDate:   o.endDate   ?? "", endTime:   o.endTime   ?? "",
                region:    o.region    ?? "", place:     o.place     ?? "",
                placeUrl:  o.placeUrl  ?? "",
              }))
            : [{ ...EMPTY_OPTION }]
        );
        setDiscounts(
          d.discounts?.map((dc: { id: number | null; type: string; condition: string; amount: number | null }) => ({
            id: dc.id ?? null,
            type: dc.type ?? "", condition: dc.condition ?? "",
            amount: dc.amount != null ? String(dc.amount) : "",
          })) ?? []
        );
        setAccountId(d.account?.id ?? null);
        setAccountBank(d.account?.bank ?? "");
        setAccountNo(d.account?.account ?? "");
        setAccountName(d.account?.name ?? "");
        setContacts(
          d.contacts?.map((c: { id: number | null; type: string; account: string | null; name: string | null }) => ({
            id: c.id ?? null,
            type: c.type ?? "", account: c.account ?? "", name: c.name ?? "",
          })) ?? []
        );
        setNotices(
          d.notices?.map((n: { id: number | null; type: string; text: string | null }) => ({
            id: n.id ?? null,
            type: n.type ?? "", text: n.text ?? "",
          })) ?? []
        );
      } catch (e) {
        setFetchErr(e instanceof Error ? e.message : "데이터를 불러오지 못했습니다.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    try {
      ...
      const body = {
        ...
        discounts: discounts.length > 0
          ? discounts.map((d) => ({ id: d.id ?? null, type: d.type, condition: d.condition, amount: d.amount ? Number(d.amount) : null }))
          : null,
        account: hasAccount
          ? { id: accountId, bank: accountBank || null, account: accountNo || null, name: accountName || null }
          : null,
        contacts: contacts.length > 0
          ? contacts.map((c) => ({ id: c.id ?? null, type: c.type, account: c.account || null, name: c.name || null }))
          : null,
        isActive,
        notices: notices.length > 0
          ? notices.map((n) => ({ id: n.id ?? null, type: n.type, text: n.text || null }))
          : null,
      };

      const res = await fetch(`/api/lesson/${lessonNo}`, {
        method: "PUT",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <h2 className="text-base font-semibold text-gray-900">레슨 수정 <span className="text-gray-400 font-normal text-sm">#{lessonNo}</span></h2>
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        {onAdd && <button type="button" onClick={onAdd} className="text-xs text-indigo-600 hover:text-indigo-800 font-medium">+ 추가</button>}
      </div>
      {children}
    </section>
  );
...
function RowHeader({ label, onRemove }: { label: string; onRemove?: () => void }) {
  return (
    <div className="flex items-center justify-between">
      ...
      {onRemove && <button type="button" onClick={onRemove} className="text-xs text-red-400 hover:text-red-600">삭제</button>}
    </div>
  );
}
